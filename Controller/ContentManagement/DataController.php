...
class DataController extends AppController
{
    ...
    public function viewDataAction($environmentName, $type, $ouuid)
    {
        ...
        if (!$environments || 1 != \count($environments)) {
    ...
    public function editRevisionAction($revisionId, Request $request, LoggerInterface $logger, DataService $dataService, TranslatorInterface $translator)
    {
        ...
        if ($revision->getEndTime() && !$this->isGranted('ROLE_SUPER')) {
            throw new ElasticmsException($translator->trans('log.data.revision.only_super_can_finalize_an_archive', [EmsFields::LOG_CONTENTTYPE_FIELD => $revision->getContentType()->getName(), EmsFields::LOG_OUUID_FIELD => $revision->getOuuid(), EmsFields::LOG_OPERATION_FIELD => EmsFields::LOG_OPERATION_READ, EmsFields::LOG_REVISION_ID_FIELD => $revision->getId()], EMSCoreBundle::TRANS_DOMAIN));
        }
        ...
        if ($form->isSubmitted()) {//Save, Finalize or Discard
            ...
            if (!\array_key_exists('publish', $request->request->get('revision'))) {
                if (null != $revision->getOuuid()) {
                    if (0 === \count($form->getErrors()) && $revision->getContentType()->isAutoPublish()) {
                        $this->getPublishService()->silentPublish($revision);
                    }
