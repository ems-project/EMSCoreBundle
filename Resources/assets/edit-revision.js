...
function addEventListeners(target) {
    ...
    target.find('#reorganize_addItem_value').on("select2:select", function() {
        ...
        if ($(this).val().startsWith(contentType + ':') && $('li#' + $(this).val().replace(':', '\\:')).length > 0) {
            ...
            let optionTitle = $(this).find(':selected')[0].text;
            let output = $(`<li id="${$(this).val()}" class="mjs-nestedSortable-no-nesting">
                            <div class="box-header">
                                ...
                                <h3 class="box-title">${optionTitle}</h3>
                            </div>
                        </li>`);
    ...
    target.find(".reorder-modal-btn").click(function() {

        let optionsList = $(this).parent().find('select');
        let types = optionsList.data('type');
        let output = '';

        $('#reorganize_addItem_value, #hierarchical-row').data('type', types);
        $('#reorder-modal').removeData('select-id').data('select-id', optionsList.attr('id'));

        optionsList.find(':selected').each(function() {
            let optionId = $(this).val();
            let optionTitle = $(this).attr('title');

            output += `<li id="${optionId}" class="mjs-nestedSortable-no-nesting">
                        <div class="box-header">
                            <div class="pull-right box-tools">
                                <div class="btn-group">
                                    <a type="button" class="btn btn-default btn-sm remove-item" data-toggle="tooltip" title="" data-original-title="Remove">
        ...
        $('#reorder-modal .modal-body ol').html(output);
        addEventListeners($('#reorder-modal .modal-body ol'));
