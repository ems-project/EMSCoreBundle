...
class DraftInProgress implements EntityServiceInterface
{
    ...
    public function getDataTable(string $ajaxUrl, ?ContentType $contentType): EntityTable
    {
        ...
        $table->addColumnDefinition(new DatetimeTableColumn('revision.draft-in-progress.column.created', 'created'));
        ...
        $table->addDynamicItemGetAction(Routes::ViewRevision, 'revision.draft-in-progress.column.view-revision', 'archive', [
            'type' => 'contentType.name',
            'ouuid' => 'ouuid',
        ])->addCondition(new NotEmpty('ouuid'));
        $inMyCircles = new InMyCircles($this->userService, $this->authorizationChecker);
        $table->addDynamicItemGetAction(Routes::EditRevision, 'revision.draft-in-progress.column.edit-draft', 'pencil', [
            'revisionId' => 'id',
        ])->addCondition($inMyCircles)->setButtonType('primary');
        $table->addDynamicItemPostAction(Routes::DiscardDraft, 'revision.draft-in-progress.column.discard-draft', 'trash', 'revision.draft-in-progress.column.confirm-discard-draft', [
           'revisionId' => 'id',
