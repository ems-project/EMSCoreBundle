 ...
 */
final class Version20190324000707 extends AbstractMigration
{
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__uploaded_asset AS SELECT id, created, modified, status, sha1, name, type, username, available, size, uploaded FROM uploaded_asset');
        $this->addSql('DROP TABLE uploaded_asset');
        $this->addSql('CREATE TABLE uploaded_asset (id INTEGER NOT NULL, created DATETIME NOT NULL, modified DATETIME NOT NULL, status VARCHAR(64) DEFAULT NULL COLLATE BINARY, name VARCHAR(1024) NOT NULL COLLATE BINARY, type VARCHAR(1024) NOT NULL COLLATE BINARY, username VARCHAR(255) NOT NULL COLLATE BINARY, available BOOLEAN NOT NULL, size BIGINT NOT NULL, uploaded BIGINT NOT NULL, sha1 VARCHAR(128) NOT NULL, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO uploaded_asset (id, created, modified, status, sha1, name, type, username, available, size, uploaded) SELECT id, created, modified, status, sha1, name, type, username, available, size, uploaded FROM __temp__uploaded_asset');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__uploaded_asset AS SELECT id, created, modified, status, sha1, name, type, username, available, size, uploaded FROM uploaded_asset');
        $this->addSql('DROP TABLE uploaded_asset');
        $this->addSql('CREATE TABLE uploaded_asset (id INTEGER NOT NULL, created DATETIME NOT NULL, modified DATETIME NOT NULL, status VARCHAR(64) DEFAULT NULL, name VARCHAR(1024) NOT NULL, type VARCHAR(1024) NOT NULL, username VARCHAR(255) NOT NULL, available BOOLEAN NOT NULL, size BIGINT NOT NULL, uploaded BIGINT NOT NULL, sha1 VARCHAR(40) NOT NULL COLLATE BINARY, PRIMARY KEY(id))');
        $this->addSql('INSERT INTO uploaded_asset (id, created, modified, status, sha1, name, type, username, available, size, uploaded) SELECT id, created, modified, status, sha1, name, type, username, available, size, uploaded FROM __temp__uploaded_asset');
        $this->addSql('DROP TABLE __temp__uploaded_asset');
        $this->addSql('DROP INDEX UNIQ_8D93D64992FC23A8');
        $this->addSql('DROP INDEX UNIQ_8D93D649A0D96FBF');
