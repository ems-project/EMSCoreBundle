...
final class Version20210907065810 extends AbstractMigration
{
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__revision AS SELECT id, content_type_id, task_current_id, created, modified, auto_save_at, deleted, version, start_time, end_time, draft, finalized_by, finalized_date, deleted_by, lock_by, auto_save_by, lock_until, raw_data, auto_save, circles, labelField, sha1, version_uuid, version_tag, archived, archived_by, ouuid, task_planned_ids, task_approved_ids, owner FROM revision');
        ...
        , version_tag VARCHAR(255) DEFAULT NULL COLLATE BINARY, archived BOOLEAN DEFAULT \'0\' NOT NULL, archived_by VARCHAR(255) DEFAULT NULL COLLATE BINARY, task_planned_ids CLOB DEFAULT NULL COLLATE BINARY --(DC2Type:json)
        ...
        $this->addSql('INSERT INTO revision (id, content_type_id, task_current_id, created, modified, auto_save_at, deleted, version, start_time, end_time, draft, finalized_by, finalized_date, deleted_by, lock_by, auto_save_by, lock_until, raw_data, auto_save, circles, labelField, sha1, version_uuid, version_tag, archived, archived_by, ouuid, task_planned_ids, task_approved_ids, owner) SELECT id, content_type_id, task_current_id, created, modified, auto_save_at, deleted, version, start_time, end_time, draft, finalized_by, finalized_date, deleted_by, lock_by, auto_save_by, lock_until, raw_data, auto_save, circles, labelField, sha1, version_uuid, version_tag, archived, archived_by, ouuid, task_planned_ids, task_approved_ids, owner FROM __temp__revision');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__uploaded_asset AS SELECT id, created, modified, status, name, type, username, available, size, uploaded, sha1, hash_algo, hidden, head_last, head_in FROM uploaded_asset');
        $this->addSql('DROP TABLE uploaded_asset');
        $this->addSql('CREATE TABLE uploaded_asset (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, created DATETIME NOT NULL, modified DATETIME NOT NULL, status VARCHAR(64) DEFAULT NULL COLLATE BINARY, name VARCHAR(1024) NOT NULL COLLATE BINARY, type VARCHAR(1024) NOT NULL COLLATE BINARY, username VARCHAR(255) NOT NULL COLLATE BINARY, available BOOLEAN NOT NULL, size BIGINT NOT NULL, uploaded BIGINT NOT NULL, sha1 VARCHAR(128) NOT NULL COLLATE BINARY, hash_algo VARCHAR(32) DEFAULT \'sha1\' NOT NULL COLLATE BINARY, hidden BOOLEAN DEFAULT \'0\' NOT NULL, head_last DATETIME DEFAULT NULL, head_in CLOB DEFAULT NULL --(DC2Type:array)
        )');
        $this->addSql('INSERT INTO uploaded_asset (id, created, modified, status, name, type, username, available, size, uploaded, sha1, hash_algo, hidden, head_last, head_in) SELECT id, created, modified, status, name, type, username, available, size, uploaded, sha1, hash_algo, hidden, head_last, head_in FROM __temp__uploaded_asset');
